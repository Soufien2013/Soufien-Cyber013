...
def forgot_password():
    ...
    tk.Label(win, text="⚠️ For account recovery, please contact the developer.", font=("Arial", 14), bg="#121212", fg="white").pack(pady=15)

    fb_image_url = "https://img.icons8.com/ios-filled/50/ffffff/facebook--v1.png"
    fb_data = urllib.request.urlopen(fb_image_url).read()
    fb_image = Image.open(io.BytesIO(fb_data)).resize((50, 50))
    fb_icon = ImageTk.PhotoImage(fb_image)

    def open_fb():
        webbrowser.open(fb_url)

    fb_btn = tk.Button(win, image=fb_icon, bg="#121212", bd=0, cursor="hand2", command=open_fb)
    fb_btn.image = fb_icon
    fb_btn.pack(pady=10)

    tk.Label(win, text="Click the icon to open Facebook.", font=("Arial", 12), bg="#121212", fg="gray").pack(pady=5)

def show_temp_popup(title, message, duration=300):
    popup = tk.Toplevel()
    popup.title(title)
    popup.geometry("300x100")
    popup.configure(bg="#121212")
    tk.Label(popup, text=message, font=("Arial", 14), bg="#121212", fg="white").pack(expand=True)
...
def login():
    ...
    if username == VALID_USERNAME and password == VALID_PASSWORD:
        show_temp_popup("Access Granted", "✅ Access Granted", duration=300)
        login_window.after(300, lambda: [login_window.destroy(), open_main_window()])
    else:
        show_temp_popup("Access Denied", "❌ Access Denied", duration=800)
...
tk.Label(login_window, text="🎩 Welcome White Hat 🎩", font=("Helvetica", 20, "bold"), bg="#121212", fg="#00ffcc").pack(pady=10)
tk.Label(login_window, text="Authenticate to access your toolkit", font=("Arial", 12), bg="#121212", fg="gray").pack(pady=5)
...
tk.Button(login_window, text="Login", command=login, font=("Arial", 13, "bold"), bg="#00cc66", fg="white", width=15, pady=5).pack(pady=10)
tk.Button(login_window, text="Forgot Password?", command=forgot_password, font=("Arial", 11), bg="#444", fg="white", width=15).pack()
